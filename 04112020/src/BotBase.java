...
public class BotBase extends Bot {
    ...
    public void textMessage(Message message) {
        ...
        Comando c = Comando.fromString(s);

        switch (c) {
            case SALUTA:
                if (message.getChat().getUserName() == null) {
                    MessageToSend mts = new MessageToSend(message.getChat().getId(), "Ciao! " + message.getFrom().getLastName());
                    sendMessage(mts);
                } else {
                    MessageToSend mts = new MessageToSend(message.getChat().getId(), "Ciao! " + message.getFrom().getUserName());
                    sendMessage(mts);
                }
                break;
            case TASTIERA:
                ...
                break;
            case MONOPATTINO:
                mts = new MessageToSend(message.getFrom().getId(), "rimuovo la tastiera");
                mts.setReplyMarkup(new ReplyKeyboardRemove());
                mts = new MessageToSend(message.getChat().getId(), "Es semplice");
                sendMessage(mts);
                break;
            case SCOOTER:
                mts = new MessageToSend(message.getChat().getId(), "Es medio");
                sendMessage(mts);
                break;
            case MOTO:
                mts = new MessageToSend(message.getChat().getId(), "Es medio");
